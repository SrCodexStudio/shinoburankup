...
class GuiConfigManager(private val plugin: JavaPlugin) {
    ...
    // Per-status head textures (optional, used when set to non-blank)
    fun ranksCompletedHeadTexture(): String? = ranksConfig.getString("rank-items.completed.head-texture")?.takeIf { it.isNotBlank() }
    fun ranksCurrentHeadTexture(): String? = ranksConfig.getString("rank-items.current.head-texture")?.takeIf { it.isNotBlank() }
    fun ranksAvailableHeadTexture(): String? = ranksConfig.getString("rank-items.available.head-texture")?.takeIf { it.isNotBlank() }
    fun ranksLockedHeadTexture(): String? = ranksConfig.getString("rank-items.locked.head-texture")?.takeIf { it.isNotBlank() }
    ...
    /**
     * Represents a custom decorative item placed in the ranks GUI.
     */
    ...
    /**
     * Load custom items defined in gui/ranks.yml under "custom-items".
     * Each item can be a decorative element or a PLAYER_HEAD with a custom texture.
     */
    fun ranksCustomItems(): List<CustomGuiItem> {
        val items = ranksConfig.getMapList("custom-items")
        return items.mapNotNull { map ->
            try {
                val materialName = (map["material"] as? String) ?: "STONE"
                val material = try {
                ...
                CustomGuiItem(
                    slot = (map["slot"] as? Number)?.toInt() ?: return@mapNotNull null,
                    material = material,
                    headTexture = (map["head-texture"] as? String)?.takeIf { it.isNotBlank() },
                    name = (map["name"] as? String) ?: " ",
                    lore = (map["lore"] as? List<*>)?.filterIsInstance<String>() ?: emptyList(),
